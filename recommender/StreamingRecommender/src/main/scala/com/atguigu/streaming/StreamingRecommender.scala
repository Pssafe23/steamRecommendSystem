...
object ConnHelper extends Serializable{
  lazy val jedis = new Jedis("node2")
  lazy val mongoClient = MongoClient( MongoClientURI("mongodb://node1:27017/gamerecommender") )
}
...
object StreamingRecommender {
  ...
  val MONGODB_MOVIE_RECS_COLLECTION = "GameRecs"
      ...
      rdds => rdds.foreach{
        case (uid, mid, score, timestamp) => {
          ...
          if (candidateMovies.isEmpty) {
            println(s"No similar movies found for movie $mid, please input another stream.")
          } else {
  ...
  def getTopSimMovies(num: Int, mid: Int, uid: Int, simMovies: scala.collection.Map[Int, scala.collection.immutable.Map[Int, Double]])
    ...
    if (!simMovies.contains(mid)) {
      println(s"No similar movies found for movie $mid")
    ...
    if (simMovies == null) {
      ...
      val allSimMovies = simMovies.toArray
      println(s"All similar movies for movie $mid: $allSimMovies")
      // rest of the code to get top N similar movies
    }
